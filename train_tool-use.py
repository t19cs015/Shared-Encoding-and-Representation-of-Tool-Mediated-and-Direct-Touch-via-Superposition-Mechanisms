# 手をのばしたときのデータ(OutstretchedHand)でmotionとvisionのエンコーダとデコーダを追加学習する
...
class Model(nn.Module):
    ...
    def forward(self, v, m, t):
        ...
        mask = self.gen_mask(vB, vT, 0.5).to(v.device)
        venc = mask*venc
        ...
        mask = self.gen_mask(B, T, 0.5).to(t.device)
        tactile = mask*tactile
        tactile = tactile.view(B, T, -1)

        enc = torch.cat([venc, motion, tactile], -1)
        h, _ = self.lstm(enc)
...
# データセットとデータローダーの作成
file_paths = [
    "train-data_tool-use.pkl",
]

combined_train_dataset = MyDataset(file_paths)

with open("test-data_tool-use.pkl", 'rb') as file:
...
state_dict = torch.load('result-random81-2-VM/model4.pth')
# モデルにパラメータをロード
...
# LSTM, tactile encoder, tactile decoderのパラメータを固定する
...
optimizer = optim.Adam(parameters_to_optimize, lr=0.001)
criterion = nn.MSELoss()
